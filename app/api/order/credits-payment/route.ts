...
export async function POST(req: NextRequest) {
  try {
    ...
    const order = await db.$transaction(async (tx) => {
      // ── 1. Verify user has sufficient balance (inside transaction = atomic) ──
      ...
      // ── 2. Fetch DB products to build order items ─────────────────────────
      ...
      if (existingOrder) {
        ...
        finalOrder = await tx.order.create({
          data: {
            ...
            totalAmount: grandTotal,
            paymentMethod: "CREDITS",
            paymentStatus: "PAID",        // Credits = instant, no admin approval needed
            orderStatus: "CONFIRMED",
            buyerPaidAt: new Date(),
