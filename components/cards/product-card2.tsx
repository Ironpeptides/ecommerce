...
import ProductDetailsCard from './product-details.card2';
...
const ProductCard = ({ product, isEvent, isWide = false }: { product: any; isEvent?: boolean; isWide?: boolean }) => {
  ...
  const productOptions: ProductOption[] = product?.variants?.length
    ...
    : [
        ...
        { 
          ...
          originalVariant: { value: '30mg', unit: '3ml', price: product?.salePrice * 1.5 } 
        },
        { 
          label: '60mg 3ml', 
          value: '60mg-3ml', 
          price: product?.salePrice * 2,
          originalVariant: { value: '60mg', unit: '3ml', price: product?.salePrice * 2 } 
        },
        { 
          label: '120mg – 10ml', 
          value: '120mg-10ml', 
          price: product?.salePrice * 3.5,
          originalVariant: { value: '120mg', unit: '10ml', price: product?.salePrice * 3.5 } 
        },
      ];
  ...
  return (
    <>
      <div className={`group w-full h-full flex flex-col bg-white/5 border border-white/10 rounded-xl overflow-hidden relative transition-all duration-300 hover:border-emerald-500/30 hover:shadow-[0_8px_30px_rgb(0,0,0,0.5)] ${isWide ? 'md:flex-row' : ''}`}>
        ...
        <Link
          ...
          className={`block relative overflow-hidden bg-[#121214] ${isWide ? 'md:w-1/2 md:h-auto' : 'aspect-square'}`}
        >
          <Image
        ...
        {/* Content */}
        <div className={`p-4 space-y-2 flex flex-col flex-1 ${isWide ? 'md:w-1/2' : ''}`}>
